...
### Обновление и перезапуск одной командой

Файл **`windows_variant\docker\update.cmd`** обновляет установку целиком: тянет
свежий код из GitHub (если это git-репозиторий), пересобирает образ приложения и
перезапускает контейнеры, затем печатает чек-лист. Данные сохраняются — индекс
...
Генерацию (LLM) выполняет Ollama на хосте, и по умолчанию она использует одну карту,
если модель в неё помещается. Файл **`windows_variant\docker\gpus.cmd`** задаёт Ollama
переменные для нескольких карт (`CUDA_VISIBLE_DEVICES`, `OLLAMA_SCHED_SPREAD=1`,
...
Чтобы модель реально распределилась по картам, после запуска задайте вопрос в чате
(или `ollama run <модель>`). Проверка: `ollama ps` и `nvidia-smi`. Замечание:
распределение по нескольким GPU нужно прежде всего для крупных моделей/большого
контекста и параллельных запросов; на модели, влезающей в одну карту, прироста скорости
может не быть (обмен по PCIe). Эмбеддинги/реранк в контейнере используют одну карту
(`cuda:0`) — это нормально.
