...
- Папка с документами (прайс-листы, презентации, записи обучения и т.д.).
  ...
  для .rar/.7z — системные `unar` и `libarchive-tools` (даёт `bsdtar`, умеет RAR5) и/или
  `p7zip` (или Python-пакеты py7zr/rarfile). Установщики ставят их сами.
...
2. системные пакеты: `python@3.11`, `ffmpeg` (кадры/фрагменты видео и транскрибация),
   ...
   которые не берёт `libredwg`/`dwg2dxf`) используется **ODA File Converter** как
   запасной конвертер DWG→DXF. Положите его дистрибутив (`*.deb` для Linux) в папку
   **`vendor/oda/`** репозитория — тогда инсталляторы Linux (`run_gpu.sh`, `setup_gpu.sh`,
   `update.sh`) и Docker-образ для Windows поставят его **автоматически** вместе с `xvfb`
   и Qt/X-библиотеками (ODA — Qt-приложение, на сервере без дисплея запускается через
   `xvfb-run`). Логика — `scripts/install_oda.sh`; кнопка «Администратор → Переустановка
   и сброс → Установить/проверить ODA» тоже ставит файл из `vendor/oda/`. Путь при
   необходимости — в `ODA_CONVERTER_PATH`;
...
- Контейнеру Qdrant задан высокий лимит файловых дескрипторов (`ulimits.nofile=1048576` в
  `docker-compose.gpu.yml`) — на больших коллекциях иначе появляется `Too many open files`.
  Применяется при пересоздании контейнера (`docker compose ... up -d`); проверить —
  `docker exec rag_qdrant sh -c 'ulimit -n'`. Массовая загрузка Qdrant (`QDRANT_BULK_INDEXING`,
  вкл. по умолч.) на время индексации отключает HNSW-индексацию и строит индекс в конце.
...
| `XTTS_SAMPLE` | Путь к образцу голоса (WAV 16 кГц/моно) для клонирования; задаётся при загрузке образца в UI | пусто |
| `XTTS_LANGUAGE` / `XTTS_USE_GPU` | Язык синтеза голосом-клоном / синтез на GPU (CUDA) | `ru` / `0` |
| `XTTS_MODEL` | Модель Coqui для клонирования (скачивается автоматически) | `…/xtts_v2` |
...
| `SIP_SPEAK_ANSWER` / `SIP_ACK_PHRASE` | Озвучивать ответ модели в звонке / короткая отметка, если выключено | `1` / «Ваш запрос принят…» |
| `SIP_TX_SPEEDUP` | Ускорение отправки RTP в pyVoIP (при «медленном»/рваном звуке; 0 — штатно) | `0` |
| `SIP_DEBUG` | Подробный лог pyVoIP (обмен SIP/REGISTER, вход RMS, STT, TTS) | `0` (выкл) |
| `INDEX_LLM_DESCRIBE` / `INDEX_LLM_DESCRIBE_MAXCHARS` | Описывать каждый файл LLM при индексации / сколько символов слать | `0` / `6000` |
