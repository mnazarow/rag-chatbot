...
FIELDS: list[dict] = [
    ...
    {"key": "QUERY_REWRITE", "label": "Улучшение запроса перед поиском",
     "group": "Поиск и генерация", "type": "select", "scope": "live",
     "options": ["off", "rewrite", "hyde"], "default": config.QUERY_REWRITE,
     "desc": "Как готовить запрос к векторному поиску. <code>off</code> — искать по вопросу как "
             "есть (быстро, по умолчанию). <code>rewrite</code> — LLM переформулирует вопрос в "
             "чёткий поисковый запрос (убирает «воду», раскрывает сокращения). <code>hyde</code> — "
             "LLM пишет короткий гипотетический ответ, и поиск идёт по ЕГО эмбеддингу (часто "
             "повышает полноту на размытых/непрямых вопросах). Оба режима добавляют один вызов "
             "LLM к запросу (кэшируется); ответ по-прежнему строится строго по найденным "
             "документам. Реранк и BM25 работают по исходному вопросу."},
    ...
    {"key": "LLM_READ_TIMEOUT", "label": "Таймаут ответа LLM (чтения), с",
     "group": "Генерация (LLM)", "type": "int", "scope": "live",
     "default": config.LLM_READ_TIMEOUT,
     "desc": "Сколько ждать очередной кусок ответа от модели. Если движок (vLLM/Ollama) завис и "
             "не шлёт данные дольше этого времени — запрос прерывается и освобождает слот очереди "
             "(иначе один зависший запрос вешает всю очередь: «выполняется 1/1, в очереди N»). "
             "Активная генерация таймаут не сбрасывает (обновляется на каждом токене). 0 — без "
             "таймаута (не рекомендуется). По умолчанию 180."},
    ...
    {"key": "LLM_THINK", "label": "Размышления гибридных моделей (thinking)",
     "group": "Генерация (LLM)", "type": "bool", "scope": "live", "default": config.LLM_THINK,
     "desc": "Для гибридных моделей (Qwen3/Qwen3.6, DeepSeek-R1 и т. п.) Ollama по умолчанию "
             "генерирует длинную фазу рассуждений — из-за этого видимый ответ появляется с большой "
             "задержкой или чат кажется «зависшим». <b>Выключено</b> (по умолчанию) — модель отвечает "
             "сразу, без размышлений (рекомендуется для RAG). Включите, только если нужны развёрнутые "
             "рассуждения (медленнее). Применяется только к моделям с поддержкой thinking; на "
             "остальные не влияет."},
    {"key": "HIDE_THINK_CHAT", "label": "Скрывать рассуждения <think> в веб-чате",
     "group": "Генерация (LLM)", "type": "bool", "scope": "live", "default": config.HIDE_THINK_CHAT,
     "desc": "Вырезать блок рассуждений <code>&lt;think&gt;…&lt;/think&gt;</code> из ответа в веб-чате. "
             "Модель может рассуждать (см. выше), но сами «мысли» пользователю не показываются. "
             "Включено по умолчанию. Выключите, если хотите видеть ход рассуждений в чате."},
    {"key": "HIDE_THINK_TELEGRAM", "label": "Скрывать рассуждения <think> в Телеграме",
     "group": "Генерация (LLM)", "type": "bool", "scope": "live", "default": config.HIDE_THINK_TELEGRAM,
     "desc": "То же для ответов Телеграм-бота: вырезать <code>&lt;think&gt;…&lt;/think&gt;</code>. "
             "Включено по умолчанию (независимо от настройки для веб-чата)."},
    ...
    {"key": "INDEX_LLM_DESCRIBE", "label": "Описывать каждый файл через LLM при индексации",
     ...
     "desc": "Для <b>каждого</b> файла LLM составляет краткое описание/summary (о чём документ, "
             ...
             "Требует переиндексации."},
    {"key": "STRUCTURE_CHUNK", "label": "Структурное чанкование (по заголовкам/абзацам)",
     "group": "Документы и индексация", "type": "bool", "scope": "reindex",
     "default": config.STRUCTURE_CHUNK,
     "desc": "Резать документ не по фиксированному размеру, а по <b>структуре</b>: границы "
             "чанков — на заголовках разделов и абзацах; таблицы и списки по возможности "
             "держатся целиком; слишком крупные блоки дробятся по предложениям. Единицы поиска "
             "получаются осмысленнее (особенно регламенты, инструкции, прайсы) — реже рвётся "
             "факт посередине. «Размер чанка» становится верхней границей блока. Требует "
             "переиндексации."},
    {"key": "INDEX_CONTEXTUAL", "label": "Контекстные чанки (заголовок+тема в эмбеддинг)",
     "group": "Документы и индексация", "type": "bool", "scope": "reindex",
     "default": config.INDEX_CONTEXTUAL,
     "desc": "Перед эмбеддингом к каждому чанку добавляется <b>название документа</b> и его "
             "<b>тема/продукт</b> (contextual retrieval). Чанк «Гарантия 3 года» из файла "
             "«Насос SPL-200» начинает находиться по запросу «гарантия на насос SPL». В базе "
             "хранится исходный текст (для показа), обогащается только вектор. Повышает полноту "
             "и точность; почти бесплатно. Требует переиндексации."},
    {"key": "INDEX_PARENT_CONTEXT", "label": "Small-to-big: родительский контекст",
     "group": "Документы и индексация", "type": "bool", "scope": "reindex",
     "default": config.INDEX_PARENT_CONTEXT,
     "desc": "Для каждого чанка сохраняется более крупный «родительский» фрагмент (окно "
             "соседних чанков). Поиск и реранк идут по маленькому точному чанку, а в LLM "
             "подаётся расширенный контекст вокруг него — ответы полнее и связнее, меньше "
             "«обрезанных» фактов. Немного увеличивает размер базы. Требует переиндексации."},
    {"key": "PARENT_WINDOW", "label": "Родительское окно (соседних чанков)",
     "group": "Документы и индексация", "type": "int", "scope": "reindex",
     "default": config.PARENT_WINDOW,
     "desc": "Сколько соседних чанков с каждой стороны включать в «родительский» фрагмент "
             "(при small-to-big). 1 — по одному слева/справа (втрое больше контекста); 2 — "
             "по два. Больше — полнее, но длиннее контекст LLM."},
    {"key": "INLINE_CITATIONS", "label": "Инлайн-цитаты: [Фрагмент N] у каждого утверждения",
     "group": "Поиск и генерация", "type": "bool", "scope": "live",
     "default": config.INLINE_CITATIONS,
     "desc": "Просить модель ставить в ответе после каждого факта ссылку на номер фрагмента-"
    ...
    {"key": "DIALOG_REWRITE", "label": "Учитывать контекст диалога в поиске",
     ...
     "desc": "Для уточняющих вопросов в продолжение диалога («а сколько это стоит?», «а "
             "гарантия?») переписывать вопрос в самостоятельный с учётом предыдущих реплик "
             "(«сколько стоит насос SPL-200?») — и уже по нему искать. Сам ответ строится по "
             "исходному вопросу и истории. Добавляет один вызов LLM только при наличии истории."},
    {"key": "ANSWER_VERIFY", "label": "Проверка обоснованности ответа (антигаллюцинации)",
     "group": "Поиск и генерация", "type": "select", "scope": "live",
     "options": ["off", "warn", "strict"], "default": config.ANSWER_VERIFY,
     "desc": "После генерации LLM сверяет, следует ли ответ строго из найденного контекста. "
             "<code>off</code> — без проверки. <code>warn</code> — если ответ не обеспечен "
             "источниками, к нему добавляется пометка. <code>strict</code> — при "
             "необеспеченности ответ перегенерируется со строгой инструкцией (максимум против "
             "выдумок). Добавляет один вызов LLM (и ещё один при strict); включайте на "
             "ответственных базах, где важна достоверность."},
    {"key": "REINDEX_SCHEDULE", "label": "Авто-переиндексация папки по расписанию",
     "group": "Документы и индексация", "type": "select", "scope": "live",
     "options": ["off", "hourly", "6h", "12h", "daily", "weekly"],
     "default": config.REINDEX_SCHEDULE,
     "desc": "Автоматически запускать <b>инкрементальную</b> переиндексацию папки документов по "
             "расписанию (индексируются только новые/изменённые файлы — по хэшу). Удобно, если "
             "документы кладут в папку регулярно. Крутит <code>monitor.py</code> (пока приложение "
             "запущено); интервально от последнего запуска. <code>off</code> — только вручную."},
    {"key": "INDEX_DEDUP", "label": "Дедуп документов по содержимому при индексации",
     "group": "Документы и индексация", "type": "bool", "scope": "reindex",
     "default": config.INDEX_DEDUP,
     "desc": "Файлы с ОДИНАКОВЫМ извлечённым текстом (совпадает SHA-256) индексируются один "
             "раз — последующие копии пропускаются с пометкой в логе. Убирает дубли из базы "
             "(одинаковые документы под разными именами/в разных папках), экономит место и "
             "чистит выдачу. Сверка в пределах одной индексации. По умолчанию выключено. "
             "Требует переиндексации."},
    ...
    {"key": "CAD_LLM_DESCRIBE", "label": "Описывать чертёж DWG/DXF vision-моделью (доп.)",
     "group": "OCR (распознавание текста)", "type": "bool", "scope": "reindex",
     "default": config.CAD_LLM_DESCRIBE,
     "desc": "Дополнительно к извлечению текста чертежа — отрисовать его в изображение и передать "
             "<b>vision-модели</b> за описанием (тип изделия/узла, элементы, размеры, штамп). "
             "Полезно для чертежей без текстовых надписей. Требуется мультимодальная модель "
             "(поле «Vision-модель» ниже или основная LLM) и <code>matplotlib</code>. "
             "Тяжело/медленно — по умолчанию выключено."},
    {"key": "CAD_DWG_DESCRIBE", "label": "Описывать неконвертируемый DWG по превью vision-моделью",
     "group": "OCR (распознавание текста)", "type": "bool", "scope": "reindex",
     "default": config.CAD_DWG_DESCRIBE,
     "desc": "Если <b>DWG не конвертируется</b> в DXF (нет libredwg/ODA или новая версия формата) — "
             "достать <b>встроенное в файл превью-изображение</b> и передать его "
             "<b>vision-модели</b> за описанием. Работает и когда текст из бинарника не извлёкся. "
             "Требуется мультимодальная модель (поле «Vision-модель» ниже или основная LLM). "
    ...
    {"key": "VISION_DOWNSCALE", "label": "Уменьшать изображение перед описанием",
     "group": "OCR (распознавание текста)", "type": "bool", "scope": "live",
     "default": config.VISION_DOWNSCALE,
     "desc": "Уменьшать большие фото/сканы перед отправкой vision-модели (до размера ниже). "
             "Снижает число image-токенов и память vLLM — частая причина «Server disconnected»/"
             "краша движка при индексации. Включено по умолчанию; выключите, чтобы слать "
             "изображение в исходном размере (медленнее и рискованнее для vLLM)."},
    {"key": "VISION_MAX_SIDE", "label": "Макс. сторона картинки для vision (px)",
     ...
     "desc": "До какого размера уменьшать (если уменьшение включено выше): большая сторона "
             "картинки, px. Меньше — быстрее и меньше памяти, но мельче детали. По умолчанию "
             "1536. Действует только при включённом «Уменьшать изображение перед описанием»."},
    ...
    {"key": "XTTS_MODEL", "label": "Модель XTTS", "group": "Телеграм-бот",
     ...
     "desc": "Идентификатор модели Coqui для клонирования. По умолчанию "
             ...
             "при первом синтезе)."},
    {"key": "XTTS_URL", "label": "Адрес сервиса XTTS", "group": "Телеграм-бот",
     "type": "text", "scope": "live", "default": config.XTTS_URL,
     "desc": "Адрес отдельного микросервиса XTTS (напр. <code>http://127.0.0.1:8020</code>). "
             "Сервис живёт в своём окружении с <code>coqui-tts</code>, чтобы не конфликтовать с "
             "ядром RAG. Если задан — синтез голоса-клона идёт по HTTP на этот сервис. "
             "Устанавливается автоматически при <code>INSTALL_XTTS=1</code>."},
    ...
    {"key": "ANSWER_CACHE_SEMANTIC", "label": "Семантический кэш ответов (похожие вопросы)",
     "group": "База данных и кэш", "type": "bool", "scope": "live",
     "default": config.ANSWER_CACHE_SEMANTIC,
     "desc": "Отдавать готовый ответ не только на ТОЧНО такой же вопрос, но и на близкий по "
             "смыслу (по эмбеддингу вопроса). Ловит перефразировки («сколько стоит X» ≈ «цена "
             "X»). Требует включённого «Кэшировать ответы LLM» и Redis. Порог близости — ниже."},
    {"key": "ANSWER_CACHE_SIM", "label": "Порог близости семантического кэша (0..1)",
     "group": "База данных и кэш", "type": "range", "scope": "live",
     "min": 0.80, "max": 0.99, "step": 0.01, "default": config.ANSWER_CACHE_SIM,
     "desc": "Насколько похожим должен быть вопрос, чтобы отдать кэшированный ответ. Выше — "
             "строже (меньше риск отдать ответ не на тот вопрос). Рекомендация: 0.92–0.96."},
    ...
    {"key": "WEB_PAGE_TIMEOUT", "label": "Таймаут загрузки страницы, с", "group": "Сайты (парсинг)",
     ...
     "desc": "Максимум ожидания одной страницы (и для обычной загрузки, и для браузера). Меньше — "
             ...
             "20–30 с (было 45)."},
    {"key": "WEB_JS_RECYCLE", "label": "Пересоздавать браузер каждые N страниц",
     "group": "Сайты (парсинг)", "type": "int", "scope": "live", "default": config.WEB_JS_RECYCLE,
     "desc": "Только для JS-парсинга через браузер. На длинных обходах (тысячи страниц) Chromium "
             "накапливает память даже при закрытых вкладках, что приводит к замедлению и зависанию. "
             "Периодическое пересоздание контекста освобождает память. 150 — разумно; 0 — отключить. "
             "Если парсинг «замирает» на большом сайте — уменьшите значение (напр. 80)."},
    ...
    {"key": "WEB_USE_SITEMAP", "label": "Использовать sitemap.xml", "group": "Сайты (парсинг)",
     "type": "bool", "scope": "live", "default": config.WEB_USE_SITEMAP,
     "desc": "Брать список страниц прямо из карты сайта <code>sitemap.xml</code> (и вложенных "
             "sitemap-индексов), а не только обходить ссылки. Быстрее и полнее, обходит ловушки "
             "поиска/пагинации. Если карта не найдена — обычный обход по ссылкам. Ссылку на "
             "карту берём из robots.txt и из стандартных путей."},
    {"key": "WEB_RESPECT_ROBOTS", "label": "Учитывать robots.txt", "group": "Сайты (парсинг)",
     "type": "bool", "scope": "live", "default": config.WEB_RESPECT_ROBOTS,
     "desc": "Соблюдать правила <code>robots.txt</code>: не заходить на запрещённые (Disallow) "
             "пути и выдерживать паузу <code>Crawl-delay</code>, если сайт её задаёт. Вежливо к "
             "сайту-источнику; выключите, только если парсите свой сайт и правила мешают."},
    {"key": "WEB_RESPECT_CRAWL_DELAY", "label": "Соблюдать паузу robots (Crawl-delay)",
     "group": "Сайты (парсинг)", "type": "bool", "scope": "live",
     "default": config.WEB_RESPECT_CRAWL_DELAY,
     "desc": "Выдерживать паузу <code>Crawl-delay</code> из robots.txt между страницами. "
             "<b>Выключите</b>, чтобы парсить без задержки (правила Disallow всё равно "
             "учитываются, если включён «Учитывать robots.txt»). Можно переопределить для "
             "каждого сайта отдельно (в его настройках обхода)."},
    {"key": "WEB_CRAWL_DELAY_MAX", "label": "Макс. пауза robots Crawl-delay, с",
     "group": "Сайты (парсинг)", "type": "range", "scope": "live",
     "min": 0, "max": 30, "step": 0.5, "default": config.WEB_CRAWL_DELAY_MAX,
     "desc": "Верхний предел паузы <code>Crawl-delay</code> из robots.txt. Некоторые сайты задают "
             "20+ секунд — тогда обход идёт ~1 страница/20 с и выглядит зависшим. robots "
             "соблюдается, но не медленнее этого значения. 5 с — разумно; 0 — не ограничивать "
             "(полностью доверять robots). Действует при включённой «Соблюдать паузу robots»."},
    {"key": "WEB_SITE_CONCURRENCY", "label": "Сайтов параллельно (потоков)",
     "group": "Сайты (парсинг)", "type": "int", "scope": "live",
     "default": config.WEB_SITE_CONCURRENCY,
     "desc": "Сколько сайтов парсить одновременно, каждый в своём потоке. 1 — последовательно "
             "(по одному). Больше — быстрее, когда сайтов несколько, но каждый параллельный сайт "
             "поднимает свой headless-браузер (расход памяти). Разумно 2–4."},
    {"key": "WEB_INCREMENTAL", "label": "Инкрементальный парсинг (только изменённое)",
     "group": "Сайты (парсинг)", "type": "bool", "scope": "live", "default": config.WEB_INCREMENTAL,
     "desc": "Пропускать неизменённые страницы и файлы условными запросами "
             "(ETag/Last-Modified): если сервер отвечает «не изменилось» (304) — страница берётся "
             "из кэша, файл переиспользуется с диска (без повторного скачивания и рендера). Резко "
