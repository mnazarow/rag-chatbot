...
# ВЗАИМОДЕЙСТВИЕ С LLM_MAX_CONCURRENCY: пейсинг применяется НЕЗАВИСИМО от лимита
# одновременности — даже при LLM_MAX_CONCURRENCY=0 («без ограничения») ненулевой delay
# сериализует СТАРТЫ запросов (раз в delay секунд), хотя сами запросы затем идут
# параллельно. Т.е. delay ограничивает частоту начал, а concurrency — их число «в полёте».
# Если нужна полностью параллельная работа без искусственной задержки — ставьте delay=0.
# FIXME(review): дефолт 3.0 c при concurrency=0 неявно сериализует старты и может
# удивлять («почему параллельные запросы стартуют по одному раз в 3 c»). Возможно, стоит
# сделать дефолт 0.0 и включать пейсинг только осознанно — оставлено как есть, чтобы не
# менять поведение существующих установок.
LLM_REQUEST_DELAY = _float("LLM_REQUEST_DELAY", 3.0)
# Таймаут ожидания ОЧЕРЕДНОГО куска ответа от LLM (с). Если модель/движок завис и не шлёт
# данные дольше этого времени — запрос прерывается с ошибкой, освобождая слот очереди
# (иначе один зависший запрос вешает всю очередь). Активная генерация таймаут не сбрасывает
# (сбрасывается на каждом полученном токене). 0 — без таймаута.
LLM_READ_TIMEOUT = _int("LLM_READ_TIMEOUT", 180)
# Таймаут ЧТЕНИЯ для НЕ-стриминговых запросов (stream=False, напр. chat()). Такой ответ
# приходит одним куском в конце генерации, поэтому read-таймаут должен покрывать ВСЮ
# генерацию, а не один токен (в отличие от стриминга, где он сбрасывается на каждом токене).
# Отдельный увеличенный лимит, чтобы длинные полные ответы не рвались по LLM_READ_TIMEOUT.
# 0 — без таймаута на чтение (полагаться на общий cap движка). По умолчанию 900 с.
LLM_NONSTREAM_READ_TIMEOUT = _int("LLM_NONSTREAM_READ_TIMEOUT", 900)
